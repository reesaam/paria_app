...
class AccountsPage extends CoreView<AccountsController> {
  ...
  @override
  PreferredSizeWidget? get appBar => AppAppBar(pageDetail: controller.pageDetail, barAction: _barActionIcon());
  ...
  @override
  Widget? get floatingActionButton => AppFloatingActionButtons(icon: AppIcons.add, onPressed: controller.addRecord);
  ...
  _barActionIcon() => AppIconButton(icon: AppIcons.list.icon!, onPressed: () => goToContactsBalance());
  ...
  _tableHeader() => Padding(
      ...
      child: Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Row(children: [
            _sortButton(),
            controller.hasFilter.value ? AppIconButton(icon: AppIcons.removeFilter.icon!, onPressed: () => controller.filter.clear) : shrinkSizedBox,
            AppIconButton(icon: controller.hasFilter.value == true ? AppIcons.filter.icon! : AppIcons.noFilter.icon!, onPressed: () => controller.hasFilter.invert),
  ...
  _sortButton() => AppPopupMenu(
      ...
      listItems: List<AppPopupMenuItem>.of([
        AppPopupMenuItem(text: Texts.to.sortBy(RecordsSortType.contactAZ.value), onTapFunction: () => controller.listRecords.sortByContactAZ),
        AppPopupMenuItem(text: Texts.to.sortBy(RecordsSortType.contactZA.value), onTapFunction: () => controller.listRecords.sortByContactZA),
        AppPopupMenuItem(text: Texts.to.sortBy(RecordsSortType.amountInc.value), onTapFunction: () => controller.listRecords.sortByAmountInc),
        AppPopupMenuItem(text: Texts.to.sortBy(RecordsSortType.amountDec.value), onTapFunction: () => controller.listRecords.sortByAmountDec),
        AppPopupMenuItem(text: Texts.to.sortBy(RecordsSortType.dateInc.value), onTapFunction: () => controller.listRecords.sortByDateInc),
        AppPopupMenuItem(text: Texts.to.sortBy(RecordsSortType.dateDec.value), onTapFunction: () => controller.listRecords.sortByDateDec),
      ]));

  _tableHeaderThreeDotsButton() {
    List<AppPopupMenuItem> listItems = List<AppPopupMenuItem>.empty(growable: true);
    listItems.add(AppPopupMenuItem(
        text: controller.filter.value.cleared == true ? Texts.to.accountsTableTitleMenuHideCleared : Texts.to.accountsTableTitleMenuShowCleared, onTapFunction: () => controller.changeShowCleared()));
    if (controller.filter.value.positives == true || controller.filter.value.negatives == true) {
      listItems.add(AppPopupMenuItem(text: Texts.to.accountsTableTitleMenuShowAll, onTapFunction: () => controller.showAllRecords()));
    } else {
      listItems.add(AppPopupMenuItem(text: Texts.to.accountsTableTitleMenuShowPositives, onTapFunction: () => controller.showPositive()));
      listItems.add(AppPopupMenuItem(text: Texts.to.accountsTableTitleMenuShowNegatives, onTapFunction: () => controller.showNegative()));
    }
    return AppPopupMenu(listItems: listItems);
  }

  Widget _noRecords() => Container(width: Get.width, alignment: Alignment.center, padding: AppPaddings.accountsNoRecordText, child: Text(Texts.to.accountsNoRecord));
}
